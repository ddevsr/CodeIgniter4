...
class Forge extends BaseForge
{
    ...
    protected function _alterTable(string $alterType, string $table, $field)
    {
        ...
        if ($alterType === 'DROP') {
            $fields = array_map(fn ($field) => $this->db->escapeIdentifiers(trim((string) $field)), is_string($field) ? explode(',', $field) : $field);
        ...
        for ($i = 0, $c = count($field); $i < $c; $i++) {
            if ($alterType === 'MODIFY') {
                ...
                $wantToAddNull   = ! str_contains((string) $field[$i]['null'], ' NOT');
    ...
    protected function _attributeAutoIncrement(array &$attributes, array &$field)
    {
        if (! empty($attributes['AUTO_INCREMENT']) && $attributes['AUTO_INCREMENT'] === true
            && stripos((string) $field['type'], 'NUMBER') !== false
    ...
    protected function _processColumn(array $field): string
    {
        ...
        if ($field['type'] === 'VARCHAR2' && str_starts_with((string) $field['length'], "('")) {
            $constraint = ' CHECK(' . $this->db->escapeIdentifiers($field['name'])
                . ' IN ' . $field['length'] . ')';

            $field['length'] = '(' . max(array_map('mb_strlen', explode("','", mb_substr((string) $field['length'], 2, -2)))) . ')' . $constraint;
