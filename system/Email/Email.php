...
class Email
{
    ...
    {
        $from = str_replace(['>', '<'], '', (string) $this->headers['Return-Path']);
     ...
     * @return bool
     */
    protected function attachmentsHaveMultipart(mixed $type)
    {
    ...
    protected function appendAttachments(&$body, $boundary, $multipart = null)
    {
        foreach ($this->attachments as $attachment) {
            ...
            $name = $attachment['name'][1] ?? basename((string) $attachment['name'][0]);
