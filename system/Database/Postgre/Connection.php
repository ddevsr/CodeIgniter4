...
class Connection extends BaseConnection
{
    ...
    protected function _indexData(string $table): array
    {
        ...
        foreach ($query as $row) {
            ...
            $_fields     = explode(',', preg_replace('/^.*\((.+?)\)$/', '$1', trim((string) $row->indexdef)));
            $obj->fields = array_map(static fn ($v) => trim($v), $_fields);

            if (str_starts_with((string) $row->indexdef, 'CREATE UNIQUE INDEX pk')) {
                $obj->type = 'PRIMARY';
            } else {
                $obj->type = (str_starts_with((string) $row->indexdef, 'CREATE UNIQUE')) ? 'UNIQUE' : 'INDEX';
            }
