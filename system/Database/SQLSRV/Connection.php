...
class Connection extends BaseConnection
{
    ...
    protected function _indexData(string $table): array
    {
        ...
        foreach ($query as $row) {
            ...
            $_fields     = explode(',', trim((string) $row->index_keys));
            $obj->fields = array_map(static fn ($v) => trim($v), $_fields);

            if (str_contains((string) $row->index_description, 'primary key located on')) {
                $obj->type = 'PRIMARY';
            } else {
                $obj->type = (str_contains((string) $row->index_description, 'nonclustered, unique')) ? 'UNIQUE' : 'INDEX';
            }
